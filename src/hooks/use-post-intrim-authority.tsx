"use client";
import { useCreateInterimAuthorityMutation } from "@/redux/features/license-api-slice";
...
function usePostInterimAuthority() {
  const [create, { isLoading }] = useCreateInterimAuthorityMutation();
  ...
  const validationSchema: Record<string, Yup.AnyObjectSchema> = {
    ...
    B: Yup.object({
      ...
      location: Yup.string()
        ...
        .notOneOf([""], "Please select an option"),
      has_title_deed: Yup.string()
        .required("Has title deed is required")
        .notOneOf([""], "Please select an option"),
      title_deed: Yup.string().when("has_title_deed", {
        is: (val: boolean) => val === true, // ensure it only runs when strictly true
        then: (schema) =>
          schema.required(
            'Title deed is required when "Has title deed" is yes',
          ),
        otherwise: (schema) => schema.notRequired(),
      }),
      names_of_promoters: Yup.string().required(
        "Names of promoters is required",
      ),
    }),
    ...
    C: Yup.object({
      ...
      governance_structure: Yup.string().required(
      ...
      programmes: Yup.mixed()
        .required("programmes are required")
  ...
  const onSubmit = async (values: InterimAuthority) => {
    console.log(values);
...
export default usePostInterimAuthority;
