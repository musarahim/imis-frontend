...
import { useRetrieveProgressedToManagementDetailsQuery } from "@/redux/features/programme-api-slice";
...
function Content({ id }: { id: string }) {
  const { data, isLoading, isError } =
    useRetrieveProgressedToManagementDetailsQuery(Number(id), {
    ...
    <div className="bg-white dark:bg-gray-950 rounded-lg border p-2 h-full">
      ...
      <Table className="mt-1 w-full table-fixed">
        ...
        <TableBody>
          ...
          <TableRow>
            <TableCell className={`${labelCellClassName} text-md`} colSpan={2}>
      ...
      {data?.director_comment ? (
        <div className="bg-yellow-50 dark:bg-yellow-900/20 text-yellow-800 dark:text-yellow-300 rounded-md p-4 my-4 mx-3">
          <h3 className="font-semibold mb-2">
            Management&apos;s Recommendation: {data?.status}
          </h3>
          <h3 className="font-semibold mb-2">Comment from Management:</h3>
