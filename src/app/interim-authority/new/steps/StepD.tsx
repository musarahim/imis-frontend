...
function StepD({ onBack, data, onNext }: StepDProps) {
  const [patchInterimAuthority, { isLoading }] =
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    Object.entries(values).forEach(([key, value]) => {
      if (fileFields.includes(key as keyof FormValues)) {
        ...
        if (value instanceof File) {
          ...
          value === null &&
          initialValuesRef.current[key as keyof FormValues]
        ) {
    ...
    try {
      const res = await patchInterimAuthority({
        id: Number(data?.id) || 0,
        data: formdata,
      }).unwrap();
      if (res) {
        toast.success(
          "Details saved successfully. Please preview your application before submission.",
        );
        onNext(res);
      } else {
    ...
    >
      ...
      <div className="mt-3 grid grid-cols-1 gap-x-3 gap-y-6 sm:grid-cols-6">
        <div className="sm:col-span-full">
          <FileField
            name="promoters"
            label="Names and Signatures of the promoters"
            required
          />
        </div>
        <div className="sm:col-span-full">
          <label>
            Please attach a detailed project proposal containing the following:
          </label>
