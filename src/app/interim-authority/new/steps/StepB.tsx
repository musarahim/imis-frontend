...
function StepB({ onNext, onBack, id }: StepBProps) {
  const [createInterimAuthority, { isLoading: isCreating }] =
    useCreateInterimAuthorityMutation();
  //fetch initial values if id is provided
  const { data: initialValues } = useRetrieveInterimAuthorityQuery(
    id ?? skipToken,
  );
  const [patchInterimAuthority, { isLoading: isPatching }] =
    usePatchInterimAuthorityMutation();
  ...
  const stepBValidation = Yup.object({
    ...
    title_deed: Yup.mixed().when(
      "has_title_deed",
      (has_title_deed: string[], schema: Yup.MixedSchema) => {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      if (id) {
        // update existing
        res = await patchInterimAuthority({ id, data: formData }).unwrap();
      } else {
        // create new
    ...
    >
      ...
      <div className="mt-3 grid grid-cols-1 gap-x-3 gap-y-6 sm:grid-cols-6">
        <div className="sm:col-span-full">
          <RadioInputField
            name="has_title_deed"
            label="Do you already have a title deed?"
            options={options}
          />
        </div>
        <div className="sm:col-span-full">
          <FileField
            name="title_deed"
            label="Please attach a photocopy of the land title"
            required
          />
        </div>
        <div className="sm:col-span-full">
          <RichEditorField
            name="infrastructure"
            label="Describe the existing infrastructure to be used"
            required
          />
        </div>
        ...
        <div className="sm:col-span-full">
          <TextAreaField
            ...
            label="Name the promoters of the University project"
            required
          />
        </div>
      </div>
