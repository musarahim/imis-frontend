...
import { usePatchInterimAuthorityMutation } from "@/redux/features/license-api-slice";
import { toast } from "react-toastify";
import * as Yup from "yup";
type StepCProps = {
  onBack: () => void;
  onNext: (data?: InterimAuthority) => void;
...
function StepC({ onBack, onNext, data }: StepCProps) {
  const [patchInstitution, { isLoading }] = usePatchInterimAuthorityMutation();
    ...
    >
      ...
      <div className="mt-3 grid grid-cols-1 gap-x-3 gap-y-6 sm:grid-cols-6">
        <div className="sm:col-span-full">
          <TextAreaField
            name="vision"
            label="State the Vision of the University to be established"
            type="file"
            required
          />
        </div>
        ...
        <div className="sm:col-span-full">
          <TextAreaField
            name="mission"
            label="State the Mission for which the University is to be established"
            required
          />
        </div>
        <div className="sm:col-span-full">
          <TextAreaField
            name="objectives"
            label="State the objectives for which the University is to be established"
            required
          />
        </div>
        <div className="sm:col-span-full">
          <TextAreaField
            name="philosophy"
            label="State the philosophy of the University to be established"
            required
          />
        </div>
        <div className="sm:col-span-full">
          <RichEditorField
            ...
            label="State the proposed Governance Structures of the University"
            required
          />
        </div>
        <div className="sm:col-span-full">
          <RichEditorField
            ...
            label="State the Proposed human resources that will be required to operate the University"
            required
          />
        </div>
        <div className="sm:col-span-full">
          <TextAreaField
            ...
            label="State the expected sources of financial resources"
            required
          />
        </div>
        <div className="sm:col-span-full">
          <RichEditorField
            name="action_plan"
            label="State the action plan towards the realization of the objectives of the project"
            required
          />
        </div>
        <div className="sm:col-span-full">
          <RichEditorField
            name="programmes"
            label="What programmes of study are planned in the setting up of the University"
            required
          />
        </div>
      </div>
