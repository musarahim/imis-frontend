...
import { useGetDirectorReviewsQuery } from "@/redux/features/appraisal-api-slice";
import { ColumnDef, ColumnFiltersState, PaginationState, Row, SortingState, VisibilityState } from "@tanstack/react-table";
    ...
    <div className="flex justify-center">
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          <DropdownMenuItem onClick={() => router.push(`/hr/performance_appraisal/director-reviews/${row.original.id}/review`)}>Review</DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
    </div>
  );
...
const columns: ColumnDef<PerformanceAppraisal>[] = [
  ...
  { accessorKey: "overall_level", header: ({ column }) => <DataTableColumnHeader column={column} title="Level" />, cell: ({ row }) => row.original.overall_level ? <LinkAsBadge className="bg-amber-500 text-white">{row.original.overall_level}</LinkAsBadge> : "—" },
  { id: "actions", cell: ({ row }) => <ActionsCell row={row} /> },
];

function DirectorReviewData() {
  ...
  const { data, isLoading, isError } = useGetDirectorReviewsQuery(queryParams, { refetchOnMountOrArgChange: true });
...
export default DirectorReviewData;
