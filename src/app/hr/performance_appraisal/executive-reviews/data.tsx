...
import { useGetExecutiveReviewsQuery } from "@/redux/features/appraisal-api-slice";
    ...
    <div className="flex justify-center">
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          <DropdownMenuItem onClick={() => router.push(`/hr/performance_appraisal/executive-reviews/${row.original.id}/review`)}>
            Review
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
...
const columns: ColumnDef<PerformanceAppraisal>[] = [
  ...
  { accessorKey: "overall_level", header: ({ column }) => <DataTableColumnHeader column={column} title="Level" />, cell: ({ row }) => row.original.overall_level ? <LinkAsBadge className="bg-orange-500 text-white">{row.original.overall_level}</LinkAsBadge> : "—" },
  { id: "actions", cell: ({ row }) => <ActionsCell row={row} /> },
];

function ExecutiveReviewData() {
  ...
  const { data, isLoading, isError } = useGetExecutiveReviewsQuery(queryParams, { refetchOnMountOrArgChange: true });
...
export default ExecutiveReviewData;
